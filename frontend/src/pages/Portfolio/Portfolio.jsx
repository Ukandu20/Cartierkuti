...
const FILTER = ["All Projects", "Favorites Only", "Recent Projects"];
const PAGE_SIZE = 6;
...
export default function Portfolio() {
  ...
  const fieldBg            = colorMode === "light" ? "white"    : "whiteAlpha.100";
  ...
  const navBg              = colorMode === "light" ? "gray.100" : "whiteAlpha.100";
  const nonSelectedHoverBg = colorMode === "light" ? "gray.200" : "whiteAlpha.200";
  ...
  const filterOptions = useMemo(
    () =>
      createListCollection({
        items: FILTER.map((f) => ({ label: f, value: f })),
      }),
    []
  );
  ...
  const SkeletonLoader = () => (
    <Stack borderRadius="md" bg={skeletonBg} w="100%" maxW="sm" boxShadow="sm" spacing={4} p={4}>
  ...
  if (filter === "Favorites Only") list = list.filter(p => fav.includes(p.id));
  list.sort((a, b) => {
    if (sort === "date")  return new Date(b.date) - new Date(a.date);
    if (sort === "views") return (b.views ?? 0) - (a.views ?? 0);
    return a.title.localeCompare(b.title);
  });
  const pagesCount = Math.max(1, Math.ceil(list.length / PAGE_SIZE));
  ...
  return (
    <div className={classes.container}>
      <Helmet><title>Portfolio | Preston</title></Helmet>
      ...
      >
        {/* Tab list */}
        <Tabs.List justifyContent="center" className={classes.tabs} bg={navBg}>
          {TABS.map((t, i) => (
            <Tabs.Trigger
              key={i}
              value={String(i)}
              ...
              bg="transparent"
              border="none"
              _hover={{ color: accent, bg: nonSelectedHoverBg }}
        ...
        >
          <Select.HiddenSelect />
          <Select.Label srOnly>Filter projects</Select.Label>
          <Select.Control
            bg={fieldBg}
            color={fieldTx}
            borderColor={fieldBd}
        ...
        {/* Animated content for the active tab */}
